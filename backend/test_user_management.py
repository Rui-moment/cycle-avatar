...
class UserManagementTester:
    ...
    def test_get_user_profile(self) -> bool:
        ...
        if response.status_code == 200:
            ...
            print(
                f"✅ Profile retrieved: {profile['display_name']} ({profile['email']})"
            )
            return True
        else:
    ...
    def test_update_user_profile(self) -> bool:
        ...
        update_data = {"display_name": "Updated Test User", "preferred_language": "ja"}
        ...
        if response.status_code == 200:
            ...
            print(
                f"✅ Profile updated: {updated_profile['display_name']}, language: {updated_profile['preferred_language']}"
            )
            return True
        else:
    ...
    def test_change_password(self) -> bool:
        ...
        response = self.make_request(
            "POST", "/users/me/change-password", password_data, use_auth=True
        )
    ...
    def test_user_statistics(self) -> bool:
        ...
        if response.status_code == 200:
            stats = response.json()
            print(
                f"✅ Statistics retrieved: Login count: {stats['login_count']}, Account age: {stats['account_age_days']} days"
            )
            return True
        else:
    ...
    def test_password_reset_request(self) -> bool:
        ...
        response = self.make_request(
            "POST", "/auth/password-reset-request", reset_request
        )
