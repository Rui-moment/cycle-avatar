...
def test_workout_api():
    ...
    try:
        ...
        print(
            "   ❌ Cannot connect to server. Make sure the backend is running on localhost:8000"
        )
        return
    ...
    response = requests.post(
        f"{BASE_URL}/workouts/sessions", json=session_data, headers=headers
    )
    ...
    response = requests.put(
        f"{BASE_URL}/workouts/sessions/{session_id}", json=update_data, headers=headers
    )
    if response.status_code == 200:
    ...
    response = requests.post(
        f"{BASE_URL}/workouts/sync", json=sync_data, headers=headers
    )
    if response.status_code == 200:
    ...
    response = requests.delete(
        f"{BASE_URL}/workouts/sessions/{session_id}", headers=headers
    )
