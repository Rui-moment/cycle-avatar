from sqlalchemy import Column, String, DateTime, Boolean, Text, Integer, Numeric, ForeignKey, JSON
...
class MuscleGroup(Base):
    ...
    name_en = Column(String, nullable=False)
    name_ja = Column(String, nullable=False)
    recovery_tau = Column(Numeric(5, 2), nullable=False)  # Recovery time constant in hours
...
class Set(Base):
    ...
    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()), index=True)
    session_id = Column(String(36), ForeignKey("workout_sessions.id"), nullable=False, index=True)
...
class FatigueEvent(Base):
    ...
    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()), index=True)
    muscle_group_id = Column(String(36), ForeignKey("muscle_groups.id"), nullable=False, index=True)
    fatigue_score = Column(Numeric(8, 2), nullable=False)
    timestamp = Column(DateTime(timezone=True), nullable=False)
    workout_session_id = Column(String(36), ForeignKey("workout_sessions.id"), nullable=False, index=True)
...
class RecoveryState(Base):
    ...
    readiness_level = Column(String, nullable=False, default="ready")  # ready, warm, fatigued
