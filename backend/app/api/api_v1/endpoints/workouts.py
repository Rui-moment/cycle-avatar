...
from app.models.workout import WorkoutSession, Set, Exercise, MuscleGroup, FatigueEvent, RecoveryState, PRRecord, Template
from app.api.api_v1.endpoints.auth import get_current_user
from app.schemas.workout import (
    WorkoutSessionCreate, WorkoutSessionUpdate, WorkoutSessionResponse,
    SetCreate, SetUpdate, SetResponse,
    ExerciseResponse, MuscleGroupResponse,
    BatchSyncRequest, BatchSyncResponse, SyncEntity, SyncConflict,
    RecoveryStateResponse, FatigueEventCreate, FatigueEventResponse,
    PRRecordCreate, PRRecordResponse,
...
def get_workout_sessions(
    ...
    sessions = query.options(
        joinedload(WorkoutSession.sets).joinedload(Set.exercise)
    ).order_by(desc(WorkoutSession.start_time)).offset(skip).limit(limit).all()
...
def create_workout_session(
    ...
    session_with_sets = db.query(WorkoutSession).options(
        joinedload(WorkoutSession.sets).joinedload(Set.exercise)
    ).filter(WorkoutSession.id == db_session.id).first()
...
def add_set_to_session(
    ...
    set_with_exercise = db.query(Set).options(
        joinedload(Set.exercise)
    ).filter(Set.id == db_set.id).first()
...
def update_set(
    ...
    db_set = db.query(Set).join(WorkoutSession).filter(
        and_(Set.id == set_id, WorkoutSession.user_id == current_user.id)
    ).first()
...
def get_recovery_states(
    ...
    recovery_states = db.query(RecoveryState).options(
        joinedload(RecoveryState.muscle_group)
    ).filter(RecoveryState.user_id == current_user.id).all()
...
def _sync_workout_session(entity: SyncEntity, user_id: str, db: Session) -> dict:
    ...
    if entity.action == "create":
        ...
        existing = db.query(WorkoutSession).filter(
            WorkoutSession.id == entity.entity_id
        ).first()
...
def _get_server_changes_since(last_sync: datetime, user_id: str, db: Session) -> List[SyncEntity]:
    """Get server-side changes since last sync timestamp."""
    changes = []
    
    # Get updated sessions
    updated_sessions = db.query(WorkoutSession).filter(
        WorkoutSession.user_id == user_id,
        WorkoutSession.updated_at > last_sync
    ).all()
    ...
    # Get updated sets
    updated_sets = db.query(Set).join(WorkoutSession).filter(
        WorkoutSession.user_id == user_id,
...
def get_pr_records(
    ...
    query = db.query(PRRecord).options(
        joinedload(PRRecord.exercise)
...
@router.post("/pr-records", response_model=PRRecordResponse, status_code=status.HTTP_201_CREATED)
def create_pr_record(
    ...
    # Load with exercise relationship
    pr_with_exercise = db.query(PRRecord).options(
        joinedload(PRRecord.exercise)
    ).filter(PRRecord.id == pr_record.id).first()
